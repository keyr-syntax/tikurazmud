...
import {
  ...
  type changePasswordInterface,
} from "@/store/patientAuthStore";
import { handleChangePatientPassword } from "@/services/patientAuthService";

function ChangePatientPassword() {
  const [showOldPassword, setShowOldPassword] = useState<boolean>(false);
  const [showNewPassword, setShowNewPassword] = useState<boolean>(false);
  const { loading } = changePatientPasswordStore();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const submitPatientPassword = async (formData: changePasswordInterface) => {
    const success = await handleChangePatientPassword(formData);
    if (success) {
  ...
  return (
    <>
      <AlertDialog>
        <AlertDialogTrigger>Change password</AlertDialogTrigger>
        <AlertDialogContent className="bg-[#151533]">
          <form
            onSubmit={handleSubmit(submitPatientPassword)}
