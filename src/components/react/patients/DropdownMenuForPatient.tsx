...
function DropdownMenuForPatient() {
  const {
    isAuthenticated,
    isPatient,
    isPhysician,
        ...
        username !== null && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <div className="flex items-center space-x-3 ml-auto mr-16">
                <div className="w-8 h-8 flex items-center justify-center rounded-full bg-blue-600 text-white font-semibold text-lg cursor-pointer">
                  {profilePicture ? "" : getFirstLetterOfName(username)}
                </div>
            ...
            <DropdownMenuContent className="w-56 my-4 mr-2 bg-[#151533]">
              <DropdownMenuLabel
                onClick={() => {
                  window.location.href = "/patients/patient-profile";
                }}
...
export default DropdownMenuForPatient;
