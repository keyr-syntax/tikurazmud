...
import { handleAdminLogin } from "@/services/adminAuthService";
import { loginAdminStore, type loginAdminType } from "@/store/adminAuthStore";

function LoginAdmin() {
  const [showPassword, setShowPassword] = useState<boolean>(false);
  const { loading } = loginAdminStore();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const submitAdminLoginData = async (formData: loginAdminType) => {
